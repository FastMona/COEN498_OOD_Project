...
if ~isfile(modelFile)
    error('Saved model not found: %s\nRun the last-layer training script first.', modelFile);
end

%% LOAD MODEL

fprintf('Loading trained last-layer model + MD stats...\n');
...
if ~isfield(mdStats, 'featureLayer')
    error('This model does not contain mdStats.featureLayer.');
end

fprintf('Using MD feature layer: %s\n', mdStats.featureLayer);
...
for d = 1:numel(datasets)
    ...
    %% LAST-LAYER MAHALANOBIS OOD SCORES

    fprintf('\nComputing last-layer Mahalanobis OOD scores for %s...\n', datasetName);
    ...
    summaryResults(d).nearestClassIdx = nearestClassIdx;

end

%% COMBINED HISTOGRAM OF MD SCORES FOR ALL TEST SETS

figure('Name', 'Last-Layer MD + AUROC: Mahalanobis Score Histogram for All Test Sets', ...
       'NumberTitle', 'off');
...
ylabel('Probability');
title('Last-Layer Mahalanobis Score Distribution');
legend('show');
...
fprintf('\n========================================\n');
fprintf('OVERALL OOD AUROC USING LAST-LAYER MAHALANOBIS SCORES\n');
fprintf('========================================\n');
...
figure('Name','Overall OOD ROC Curve - Last-Layer MD','NumberTitle','off');
plot(rocX, rocY, 'LineWidth', 2);
grid on;
xlabel('False Positive Rate');
ylabel('True Positive Rate');
title(sprintf('Overall OOD ROC Curve Using Last-Layer MD | AUROC = %.4f', oodAUROC));
...
resultFile = fullfile(modelFolder, 'cnn_test_results_lastlayer_md_auroc.mat');
